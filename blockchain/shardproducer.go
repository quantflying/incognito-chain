...
func (blockGenerator *BlockGenerator) NewBlockShard(shardID byte, round int, crossShards map[byte]uint64, beaconHeight uint64, start time.Time) (*ShardBlock, error) {
	...
	beaconHashes, err := rawdbv2.GetBeaconBlockHashByIndex(blockGenerator.chain.GetBeaconChainDatabase(), beaconHeight)
	if err != nil {
		return nil, err
	}
	beaconHash := beaconHashes[0]
	beaconBlockBytes, err := rawdbv2.GetBeaconBlockByHash(blockGenerator.chain.GetBeaconChainDatabase(), beaconHash)
	if err != nil {
		return nil, err
	}
	...
	if epoch-shardBestState.Epoch >= 1 {
		...
		newBeaconHashes, err := rawdbv2.GetBeaconBlockHashByIndex(blockGenerator.chain.GetBeaconChainDatabase(), beaconHeight)
		if err != nil {
			return nil, err
		}
	...
	beaconBlocks, err := FetchBeaconBlockFromHeight(blockGenerator.chain.GetBeaconChainDatabase(), shardBestState.BeaconHeight+1, beaconHeight)
	if err != nil {
		return nil, err
	}
	...
	txsToAddFromBlock, err := blockGenerator.getTransactionForNewBlock(&tempPrivateKey, shardID, beaconBlocks, blockCreationLeftOver, beaconHeight)
	if err != nil {
		return nil, err
	}
...
func (blockGenerator *BlockGenerator) getTransactionForNewBlock(privatekey *privacy.PrivateKey, shardID byte, beaconBlocks []*BeaconBlock, blockCreation int64, beaconHeight uint64) ([]metadata.Transaction, error) {
...
func (blockGenerator *BlockGenerator) getCrossShardData(toShard byte, lastBeaconHeight uint64, currentBeaconHeight uint64, crossShards map[byte]uint64) map[byte][]CrossTransaction {
	...
	for fromShard, crossShardBlock := range allCrossShardBlock {
		...
		for index, crossShardBlock := range crossShardBlock {
			...
			nextHeight, err := rawdbv2.GetCrossShardNextHeight(blockGenerator.chain.GetBeaconChainDatabase(), fromShard, toShard, startHeight)
			if err != nil {
				break
			}
			...
			beaconConsensusStateRootHash, err := blockGenerator.chain.GetBeaconConsensusRootHash(blockGenerator.chain.GetBeaconChainDatabase(), beaconHeight)
			if err != nil {
			...
			shardCommittee := statedb.GetOneShardCommittee(beaconConsensusStateDB, crossShardBlock.Header.ShardID)
