...
func handlePDEInsts(
	...
	for _, tradeAction := range sortedTradesActions {
		...
		newInst, err := buildInstructionsForPDETrade(actionContentBase64Str, tradeAction.ShardID, metadata.PDETradeRequestMeta, currentPDEState, beaconHeight)
		if err != nil {
			Logger.log.Error(err)
			continue
		}
	...
	for _, value := range wrKeys {
		...
		for _, action := range actions {
			contentStr := action[1]
			newInst, err := buildInstructionsForPDEWithdrawal(contentStr, shardID, metadata.PDEWithdrawalRequestMeta, currentPDEState, beaconHeight)
			if err != nil {
				Logger.log.Error(err)
				continue
			}
	...
	for _, value := range ctKeys {
		...
		for _, action := range actions {
			contentStr := action[1]
			newInst, err := buildInstructionsForPDEContribution(contentStr, shardID, metadata.PDEContributionMeta, currentPDEState, beaconHeight)
			if err != nil {
				Logger.log.Error(err)
				continue
			}
