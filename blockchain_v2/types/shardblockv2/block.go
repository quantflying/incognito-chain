...
func NewShardBlock() *ShardBlock {
	return &ShardBlock{
		...
		Body: ShardBody{
			Instructions:      [][]string{},
			CrossTransactions: make(map[byte][]crosstransaction.CrossTransaction),
...
func (shardBlock *ShardBlock) BuildShardBlockBody(instructions [][]string, crossTransaction map[byte][]crosstransaction.CrossTransaction, transactions []metadata.Transaction) {
...
func (shardBlock *ShardBlock) validateSanityData() (bool, error) {
	...
	if shardBlock.Header.Version < 2 {
		return false, fmt.Errorf("Expect Shard Block Version greater or equal than %+v but get %+v ", 2, shardBlock.Header.Version)
	}
...
func (shardBlock *ShardBlock) UnmarshalJSON(data []byte) error {
	...
	if shardBlock.Body.CrossTransactions == nil {
		shardBlock.Body.CrossTransactions = make(map[byte][]crosstransaction.CrossTransaction)
	}
	if shardBlock.Header.TotalTxsFee == nil {
...
func (shardBlock *ShardBlock) GetBlockType() string {
	return common.TYPES_SHARD_BLOCK
}
